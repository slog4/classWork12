package com.company;

import java.util.ArrayList;
import java.util.Collection;
import java.util.Collections;
import java.util.Comparator;

public class Main {

    public static void main(String[] args) {
        int id = 1;
        BidPayment woker1 = new BidPayment(7000, "Petya", 15);
        BidPayment woker2 = new BidPayment(6500, "Vasya", 21);
        HourlyPayment woker3 = new HourlyPayment(89, "Fedya", 100);
        HourlyPayment woker4 = new HourlyPayment(25, "Kiril", 10);
        System.out.println(woker1);
        System.out.println(woker2);
        System.out.println(woker3);
        PayrollPerMonth payrollPerMonth = new PayrollPerMonth(woker1, woker2, woker3);
        System.out.println(payrollPerMonth);
        payrollPerMonth.addWorkers(woker4);
        System.out.println("");
        System.out.println(payrollPerMonth);

    }


}


